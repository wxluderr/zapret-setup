@echo off
:: ТРЮК ДЛЯ АВТО-РАЗВЕРТЫВАНИЯ ОКНА НА ВЕСЬ ЭКРАН
if "%~1"=="max" goto :init
start /max "" "%~f0" max
exit /b
...
if %errorLevel% == 0 (
    ...
    :: Передаем параметр max, чтобы новое окно админа тоже открылось во весь экран
    powershell -Command "Start-Process -FilePath '%~f0' -ArgumentList 'max' -Verb RunAs"
    exit /b
)
echo.

:: 2. АВТОМАТИЧЕСКОЕ СКАЧИВАНИЕ С GITHUB
...
:: 5. АВТОМАТИЧЕСКИЙ ФИНАЛ И ПЕРЕХОД К СЛУЖБЕ
echo ==========================================================================================
